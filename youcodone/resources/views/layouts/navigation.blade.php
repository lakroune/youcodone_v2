            ...
            <div class="flex items-center">
                <a href="/" class="group relative">
                    <div class="flex items-center gap-3">
                        <!-- Icône/Logo stylisé -->
                        <div class="w-10 h-10 rounded-full border-2 border-[#FF6B35]/30 flex items-center justify-center group-hover:border-[#FF6B35] transition-all duration-300">
                            <span class="text-[#FF6B35] font-bold text-lg italic">C</span>
                        </div>
                        <div class="flex flex-col">
                            <span class="text-white text-xl font-bold italic tracking-tight leading-none group-hover:text-[#FF6B35] transition-colors duration-300">
                                Cibo
                            </span>
                            <span class="text-[#FF6B35] text-[10px] font-bold uppercase tracking-[0.3em] group-hover:text-white transition-colors duration-300">
                                Gustoso
                            </span>
                        </div>
                    </div>
                ...
                @role('client')
                    <a href="{{ route('home') }}" 
                       class="group relative text-[11px] font-medium uppercase tracking-[0.2em] text-gray-300 hover:text-white transition-all duration-300 py-2">
                        Exploration
                        <span class="absolute bottom-0 left-0 w-0 h-px bg-[#FF6B35] group-hover:w-full transition-all duration-300"></span>
                    </a>
            ...
            <div class="hidden md:flex items-center gap-6">
                ...
                @role('restaurateur')
                    ...
                    <a href="{{ route('notifications.index') }}" class="relative group">
                        <div class="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:border-[#FF6B35]/50 transition-all duration-300">
                        ...
                        @if (auth()->user()->unreadNotifications->count() > 0)
                            <span class="absolute -top-1 -right-1 w-5 h-5 bg-[#FF6B35] text-black text-[10px] font-bold rounded-full flex items-center justify-center animate-pulse">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false"
                        ...
                        <div class="text-right hidden lg:block">
                            <p class="text-xs font-bold text-white group-hover:text-[#FF6B35] transition-colors duration-300">
                                {{ Auth::user()->username }}
                            </p>
                        ...
                        <div class="relative">
                            <div class="w-11 h-11 rounded-full p-0.5 border-2 border-white/10 group-hover:border-[#FF6B35] transition-all duration-300 overflow-hidden">
                        ...
                        <svg class="w-4 h-4 text-gray-500 group-hover:text-[#FF6B35] transition-transform duration-300" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div x-show="open" 
                        ...
                        <a href="{{ route('profile.edit') }}"
                            ...
                            <svg class="w-4 h-4 text-gray-500 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="open" 
        ...
        @role('client')
            <a href="{{ route('home') }}" class="flex items-center justify-between group">
                <span class="text-2xl font-bold italic text-white group-hover:text-[#FF6B35] transition-colors">Exploration</span>
                <svg class="w-5 h-5 text-gray-600 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </a>
            <a href="{{ route('client.favoris') }}" class="flex items-center justify-between group">
                <span class="text-2xl font-bold italic text-white group-hover:text-[#FF6B35] transition-colors">Ma Sélection</span>
                <svg class="w-5 h-5 text-gray-600 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </a>
            <a href="{{ route('client.reservations') }}" class="flex items-center justify-between group">
                <span class="text-2xl font-bold italic text-white group-hover:text-[#FF6B35] transition-colors">Réservations</span>
                <svg class="w-5 h-5 text-gray-600 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </a>
        ...
        @role('restaurateur')
            <a href="{{ route('restaurateur.dashboard') }}" class="flex items-center justify-between group">
                <span class="text-2xl font-bold italic text-white group-hover:text-[#FF6B35] transition-colors">Mes Restaurants</span>
                <svg class="w-5 h-5 text-gray-600 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </a>
        ...
        @role('admin')
            <a href="{{ route('admin.gestion') }}" class="flex items-center justify-between group">
                <span class="text-2xl font-bold italic text-white group-hover:text-[#FF6B35] transition-colors">Utilisateurs</span>
                <svg class="w-5 h-5 text-gray-600 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </a>
            <a href="{{ route('admin.restaurants') }}" class="flex items-center justify-between group">
                <span class="text-2xl font-bold italic text-white group-hover:text-[#FF6B35] transition-colors">Restaurants</span>
                <svg class="w-5 h-5 text-gray-600 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </a>
        ...
        <div class="flex items-center gap-4 pb-6">
            <img src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : 'https://ui-avatars.com/api/?name=' . Auth::user()->username . '&background=FF6B35&color=fff' }}"
                class="w-12 h-12 rounded-full border-2 border-[#FF6B35]/30"
                alt="Avatar">
            <div>
                <p class="text-white font-bold">{{ Auth::user()->username }}</p>
                <p class="text-[#FF6B35] text-xs uppercase tracking-wider">{{ Auth::user()->roles->first()->name }}</p>
            </div>
        </div>

        <a href="{{ route('profile.edit') }}" class="block text-sm font-medium uppercase tracking-widest text-gray-400 hover:text-[#FF6B35] transition-colors">
            Paramètres du profil
        </a>
        
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full text-left text-sm font-medium uppercase tracking-widest text-red-400 hover:text-red-300 transition-colors flex items-center gap-2">
