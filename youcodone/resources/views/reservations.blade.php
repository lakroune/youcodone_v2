<x-app-layout>
    <div class="bg-[#050505] min-h-screen text-gray-200 font-serif pb-32">
        ...
        <header class="py-20 border-b border-white/5 text-center relative overflow-hidden">
            <div
                class="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-[#FF5F00]/5 via-transparent to-transparent">
            </div>
        ...
        <main class="max-w-6xl mx-auto px-6 mt-20">
            @if ($reservations->isEmpty())
                <div class="text-center py-40 border border-dashed border-white/10 rounded-[3rem]">
                    <p class="text-gray-600 uppercase tracking-[4px] text-xs italic mb-8">Votre registre est
                        actuellement vide</p>
                    <a href="{{ route('home') }}"
                        class="inline-flex items-center gap-4 text-[#FF5F00] text-[10px] font-black uppercase tracking-[3px] border border-[#FF5F00]/30 px-8 py-4 rounded-full hover:bg-[#FF5F00] hover:text-black transition-all">
                                ...
                                <tr class="group hover:bg-white/[0.01] transition-colors">
                                    <td class="px-8 py-10">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4
                                                    class="text-white font-bold uppercase tracking-wider group-hover:text-[#FF5F00] transition-colors">
                                                    {{ $reservation->restaurant->nom_restaurant }}
                                                </h4>
                                                <p class="text-[10px] text-gray-500 uppercase tracking-widest">
                                                    {{ $reservation->restaurant->adresse_restaurant }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-8 py-10">
                                        <div class="space-y-1">
                                            <p class="text-white font-serif italic text-lg">
                                                {{ \Carbon\Carbon::parse($reservation->date_reservation)->format('d M Y') }}
                                            </p>
                                            <p class="text-[10px] text-[#FF5F00] font-black uppercase tracking-widest">
                                                {{ $reservation->heure_reservation }}</p>
                                        </div>
                                    </td>
